...
let EDITANDO_PEDIDO_ID = null; // id do pedido em edição, ou null quando é um pedido novo
let ULTIMO_PEDIDO_EXIBIDO_ID = null; // id do último pedido criado/salvo, pros botões do painel de resultado
...
function ativarAba(nomeAba) {
  document.querySelectorAll('nav.abas button').forEach((b) => b.classList.toggle('ativa', b.dataset.aba === nomeAba));
  ['pedido', 'pedidos', 'vendas', 'compras', 'cadastros'].forEach((aba) => {
    document.getElementById('aba-' + aba).style.display = aba === nomeAba ? 'block' : 'none';
  });
}
...
function entrarModoEdicaoPedido(pedido) {
  EDITANDO_PEDIDO_ID = pedido.id_pedido;
  document.getElementById('select-cliente').value = pedido.id_cliente;
  CARRINHO = pedido.itens.map((i) => {
    const produto = PRODUTOS_CACHE.find((p) => p.id_produto === i.id_produto);
    return {
      id_produto: i.id_produto,
      nome_produto: produto ? produto.nome_produto : i.id_produto,
      quantidade: Number(i.quantidade),
      preco_venda: Number(i.preco_unitario_no_momento)
    };
  });
  renderCarrinho();

  document.getElementById('btn-criar-pedido').textContent = 'Salvar alterações do pedido';
  document.getElementById('btn-cancelar-edicao-pedido').style.display = 'block';
  document.getElementById('aviso-edicao-pedido').style.display = 'block';
  document.getElementById('resultado-pedido').style.display = 'none';

  ativarAba('pedido');
  document.getElementById('select-cliente').scrollIntoView({ behavior: 'smooth', block: 'center' });
}

function sairModoEdicaoPedido() {
  EDITANDO_PEDIDO_ID = null;
  CARRINHO = [];
  renderCarrinho();
  document.getElementById('btn-criar-pedido').textContent = 'Criar pedido e gerar mensagem';
  document.getElementById('btn-cancelar-edicao-pedido').style.display = 'none';
  document.getElementById('aviso-edicao-pedido').style.display = 'none';
}

document.getElementById('btn-cancelar-edicao-pedido').addEventListener('click', sairModoEdicaoPedido);

// Busca o pedido (com itens atuais) na lista de pendentes e entra em modo de edição.
// Usado tanto pelo botão "Editar itens" da aba Pedidos quanto pelo painel de resultado.
async function editarPedidoFluxo(idPedido) {
  const pedidos = await apiCall('listPedidos', { status: 'Aguardando confirmação' });
  const pedido = pedidos.find((p) => p.id_pedido === idPedido);
  if (!pedido) return alert('Pedido não encontrado — talvez já tenha sido confirmado ou excluído.');
  entrarModoEdicaoPedido(pedido);
}

async function excluirPedidoFluxo(idPedido) {
  if (!confirm('Excluir este pedido? Essa ação não pode ser desfeita.')) return;
  const resultado = await apiCall('excluirPedido', { id_pedido: idPedido });
  if (resultado._offline) {
    alert('Sem conexão: a exclusão foi salva localmente e será aplicada quando a internet voltar.');
  }
  if (EDITANDO_PEDIDO_ID === idPedido) sairModoEdicaoPedido();
  if (ULTIMO_PEDIDO_EXIBIDO_ID === idPedido) {
    document.getElementById('resultado-pedido').style.display = 'none';
    ULTIMO_PEDIDO_EXIBIDO_ID = null;
  }
  carregarPedidos();
}

document.getElementById('btn-editar-pedido-criado').addEventListener('click', () => editarPedidoFluxo(ULTIMO_PEDIDO_EXIBIDO_ID));
document.getElementById('btn-excluir-pedido-criado').addEventListener('click', () => excluirPedidoFluxo(ULTIMO_PEDIDO_EXIBIDO_ID));
...
document.getElementById('btn-criar-pedido').addEventListener('click', async () => {
  ...
  const editandoId = EDITANDO_PEDIDO_ID;

  const pedido = editandoId
    ? await apiCall('atualizarPedido', { id_pedido: editandoId, id_cliente: idCliente, itens })
    : await apiCall('criarPedido', { id_cliente: idCliente, itens });

  if (pedido._offline) {
    alert(editandoId
      ? 'Sem conexão: as alterações foram salvas localmente e serão aplicadas quando a internet voltar. A mensagem não pode ser gerada agora.'
      : 'Sem conexão: o pedido foi salvo localmente e será criado assim que a internet voltar. A mensagem de confirmação não pode ser gerada agora.');
    sairModoEdicaoPedido();
    return;
  }

  const idPedido = editandoId || pedido.id_pedido;
  const resultadoTexto = await apiCall('gerarTextoConfirmacaoPedido', { id_pedido: idPedido });
  ULTIMO_PEDIDO_EXIBIDO_ID = idPedido;
  document.getElementById('resultado-pedido').style.display = 'block';
  document.getElementById('resultado-pedido').querySelector('p').innerHTML = editandoId
    ? '<strong>Pedido atualizado!</strong> Envie a nova confirmação para o cliente:'
    : '<strong>Pedido criado!</strong> Envie a confirmação para o cliente:';
...
async function carregarPedidos() {
  ...
  lista.innerHTML = pedidos.map((p) => `
    <div class="cartao">
      ...
      <button data-id="${p.id_pedido}" class="btn-editar-pedido secundario">Editar itens</button>
      <button data-id="${p.id_pedido}" class="btn-excluir-pedido perigo">Excluir</button>
    </div>
  `).join('');
  ...
  document.querySelectorAll('.btn-editar-pedido').forEach((btn) => {
    btn.addEventListener('click', () => editarPedidoFluxo(btn.dataset.id));
  });
  document.querySelectorAll('.btn-excluir-pedido').forEach((btn) => {
    btn.addEventListener('click', () => excluirPedidoFluxo(btn.dataset.id));
  });
}
